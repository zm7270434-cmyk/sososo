...
impl GraphicsCaptureApi {
    ...
    pub fn new<T: GraphicsCaptureApiHandler<Error = E> + Send + 'static, E: Send + Sync + 'static>(
        ...
        // Create frame pool.
        // LOCAL PATCH (sososo): use 3 buffers instead of 1. The encoder sends each
        // captured surface to an async transcode thread; with a single buffer the
        // pool overwrites that surface before it is encoded → flicker/tearing.
        // Extra buffers keep a sent surface valid for several frames (matches the
        // Microsoft sample's double/triple-buffering guidance).
        ...
        >::new({
            ...
            move |frame, _| {
                    ...
                    || frame_content_size.Height != last_size.1.load(atomic::Ordering::Relaxed)
                {
                    ...
                    // LOCAL PATCH (sososo): 3 buffers (see the Create call above).
